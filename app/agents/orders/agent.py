...
from app.agents.orders.domain import Order, Query, evaluate

class Agent(BaseAgent):
    def __init__(self, records=None):
        self.records=[Order.model_validate(r).model_copy(deep=True) for r in (records or [])]
    ...
    async def run(self, query: QueryRequest, context: RequestContext) -> AgentResult:
        ...
        if context.role!=Role.ADMIN and requested is not None and requested not in context.store_ids:
            return AgentResult(agent='orders',request_id=context.request_id,status='escalated',
        ...
        audit=AuditEvent(request_id=context.request_id,session_id=context.session_id,user_role=context.role,
            agents_called=['orders'],tools_called=['orders.evaluate'],confidence=0,latency_ms=elapsed,final_status=status)
        data['audit']=audit.model_dump(mode='json')
        data['fixture']=fixture
        return AgentResult(agent='orders',request_id=context.request_id,status=status,
            summary='Structured orders analysis.' if status=='success' else 'Insufficient or missing domain data.',
            data=data,confidence=0,latency_ms=elapsed,tool_calls=['orders.evaluate'],
