...
class Agent(BaseAgent):
    def __init__(self, records=None):
        self.records=[Snapshot.model_validate(r).model_copy(deep=True) for r in (records or [])]
    ...
    async def run(self, query: QueryRequest, context: RequestContext) -> AgentResult:
        ...
        if context.role!=Role.ADMIN and requested is not None and requested not in context.store_ids:
            return AgentResult(agent='inventory',request_id=context.request_id,status='escalated',
        ...
        audit=AuditEvent(request_id=context.request_id,session_id=context.session_id,user_role=context.role,
            agents_called=['inventory'],tools_called=['inventory.evaluate'],confidence=0,latency_ms=elapsed,final_status=status)
        data['audit']=audit.model_dump(mode='json')
        data['fixture']=fixture
        return AgentResult(agent='inventory',request_id=context.request_id,status=status,
            summary='Structured inventory analysis.' if status=='success' else 'Insufficient or missing domain data.',
            data=data,confidence=0,latency_ms=elapsed,tool_calls=['inventory.evaluate'],
